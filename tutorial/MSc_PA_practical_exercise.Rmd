...
```{r image_grob007, fig.show = "hold", out.width = "80%", fig.align = "default", echo=FALSE}

knitr::include_graphics("https://github.com/KiranLDA/MSc_Protected_Areas_2025/blob/main/tutorial/download.png?raw=true")
...
```{r image_grob008, fig.show = "hold", out.width = "80%", fig.align = "default", echo=FALSE}

knitr::include_graphics("https://github.com/KiranLDA/MSc_Protected_Areas_2025/blob/main/Screenshot%202025-12-02%20163221.jpg?raw=true")
...
We are going to be using Species Distribution Modelling (SDM) data from students last year. We are going to use the binary presence/absence outputs where the probability of a species presence has been translated into a yes/no.

Each layer describes the spatial distribution of a feature. Here, our feature data correspond to different plant species. Specifically, if the cell (or planning unit) has a 1, a species is predicted to be present in it based on the SDM. If the cell has a 0, then it is predicted to be absent.
...
The world is a sphere, and for us to visualise it on paper as a rectangle, we need to make some transformation or "projections". This can lead to some regions becoming more stretched than others, and some regions being more compressed. The most common projection, the Mercator projection (top left), makes the northern hemisphere look larger than the southern hemisphere. We can have equal area projections (middle right) such as Molleweide that correct for this, but then make places like New Zealand look very squashed. Many datasets each use different projections, so it is important to make sure all the different files have the same extent (x and y limits), and the same resolution (pixel size)



```{r image_grob8, fig.show = "hold", out.width = "80%", fig.align = "default", echo=FALSE}

knitr::include_graphics("https://www.geoawesome.com/wp-content/uploads/2022/03/projections.jpg")
...
The datasets have the same projection. However, if they did not, we can use `project(species, cost)` to reproject the species data to match the cost data (or vice versa).
...
Resolution is also important as we need the cells to all line-up to be able to match them and make calculations.
...
It is likely the first scenario has no constraints and protects the cheapest sites first that meet the biodiversity target. However, some of these sites are already protected in scenario 2, so the algorithm has to consider other sites to meet the conservation target. The current PA system may also be sub optimal (for instance, protecting some ecosystems/species more than others), so the algorithm has to compensate by protecting more expensive sites to meet the same biodiversity targets for under-represented ecosystems/species. 
...
What is the second thing you notice? The protected areas are huge. It's important to consider whether very large protected areas make sense in human landscapes, as they could impact livelihoods and displace people. However, large protected areas can also be easier to manage and patrol, and can protect wide-ranging species better. It's important to think about these things when designing protected area networks.
...
# Going further: Using your own SDM

**This is not part of the practical**

Later, once you have your SDM you could try to find areas that capture 30% of your own species for the least cost. Or capture as much of your species as possible within a budget. This can help us think about conservation implications and trade-offs for our species.

Here are the steps you would need to achieve this:

1. Import your species distribution raster using `terra::rast()` (you could name it `my_species`). `prioritizr` allows you to use both probability maps and binary maps. You can test both if you want to see how it changes what areas you prioritise.

2. Check you have the right projection, resolution and extent using `crs()`, `res()` and `ext()`. 
  - If the projection is not the same, you can use `my_species <- project(my_species, cost)` to reproject your species layer to match the cost layer.
  - If the resolution is not the same (very likely as your SDM will be very fine resolution) you can use `aggregate(my_species, fact = 12, fun = "modal", na.rm = T)` to get the same resolution. I am guessing 12 will work, but if it does not, you can estimate factor using `factor <- res(cost)[1] / res(my_species)[1]`
  - If the extent is not the same, you can use `crop(my_species, cost)` if my_species is bigger than cost, or `extend(my_species, cost)` if my_species is smaller than cost.
  
3. Replace `features = species` with `features = my_species` in the `problem()` formulation of prioritizr and anywhere else in the code where it might be needed.
...
5. See where the prioritisation suggests protecting land for your species - What do you notice? What trade-offs are you making and why? How does the SDM thresholding affect the priority areas?

# Find out more

`prioritizr` has many very useful help pages - feel free to explore and do your own research to find out more about what you can do on the 
